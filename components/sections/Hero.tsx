...
export function Hero() {
    ...
    useEffect(() => {
        const ctx = gsap.context(() => {
            ...
            tl.fromTo(imageRef.current, { scale: 0.9, opacity: 0, rotate: -3 }, { scale: 1, opacity: 1, rotate: 0, duration: 1.2, ease: "power2.out" })
                .fromTo(titleRef.current, { y: 30, opacity: 0 }, { y: 0, opacity: 1, duration: 0.8 }, "-=0.8")
                ...
                .fromTo(socialRef.current, { y: 15, opacity: 0 }, { y: 0, opacity: 1, duration: 0.6 }, "-=0.3");
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">
                    ...
                    <div className="space-y-10 text-center lg:text-left order-2 lg:order-1">
                        <div className="space-y-6">
                            ...
                            <h1 ref={titleRef} className="text-5xl md:text-7xl lg:text-8xl font-bold tracking-tighter leading-[0.9] lg:leading-[0.85]">
                                Hi, I&apos;m <br />
                                <span className="text-primary">{personalInfo?.name || "Developer"}</span>
                            </h1>

                            <p ref={subtitleRef} className="text-lg md:text-xl text-muted-foreground max-w-xl mx-auto lg:mx-0 leading-relaxed font-medium">
                        ...
                        <div ref={ctaRef} className="flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-5">
                            <Link href="#projects" className="group relative px-8 py-4 bg-primary text-primary-foreground rounded-2xl font-bold transition-all hover:scale-105 active:scale-95 flex items-center gap-2 overflow-hidden shadow-lg shadow-primary/25">
                                <span>View Projects</span>
                                <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
                            </Link>
                            <Link href="#contact" className="px-8 py-4 bg-secondary text-secondary-foreground rounded-2xl font-bold border border-border/50 transition-all hover:bg-secondary/80 hover:scale-105 active:scale-95">
                                Contact Me
                            </Link>
                        </div>
                        ...
                        <div ref={socialRef} className="flex items-center justify-center lg:justify-start gap-6">
                            {personalInfo.socials.map((social) => {
                                ...
                                return (
                                    <a key={social.name} href={social.url} target="_blank" rel="noopener noreferrer" className="p-3 rounded-xl bg-secondary/50 hover:bg-primary hover:text-primary-foreground border border-border/50 transition-all duration-300 group hover:scale-110" aria-label={social.name}>
                    ...
                    <div className="relative order-1 lg:order-2 flex justify-center">
                        <div ref={imageRef} className="relative w-64 h-64 md:w-80 md:h-80 lg:w-112.5 lg:h-112.5">
                            {/* Decorative Rings */}
                            ...
                            <div className="relative w-full h-full rounded-[40px] overflow-hidden border-4 border-background shadow-2xl">
                                <Image src={personalInfo?.profileImage || ""} alt={personalInfo?.name || "Profile"} fill className="object-cover" priority />
